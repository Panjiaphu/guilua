(function installGroupV3IncomingRingtone(window) {
  "use strict";

  var ownerId = Math.random().toString(36).slice(2) + Date.now().toString(36);
  var channel = typeof window.BroadcastChannel === "function" ? new window.BroadcastChannel("group-v3-ringtone") : null;
  var context = null;
  var activeKey = "";
  ...
  function tone() {
    ...
    try {
      ...
      gain.gain.exponentialRampToValueAtTime(0.045, audio.currentTime + 0.02);
  ...
  function start(key) {
    ...
    if (channel) {
      ...
      heartbeatTimer = window.setInterval(function () {
        channel.postMessage({ type: "heartbeat", key: activeKey, owner: ownerId });
      }, 2000);
    }
    if (armed && leader) startTone();
  }
  ...
  if (channel) {
    channel.addEventListener("message", function (event) {
      ...
      if (message.type === "claim" || message.type === "heartbeat") {
        if (String(message.owner) < String(ownerId)) {
          leader = false;
          stopTone();
        }
  ...
  window.GroupV3IncomingRingtone = Object.freeze({ start: start, stop: stop, arm: arm });
